...
function LogViewerModal({ modalProps }: { modalProps: ModalProps }) {
    const [currentTab, setCurrentTab] = useState<"deleted" | "edited">(
        "deleted",
    );
    const [selectedChannel, setSelectedChannel] = useState<string>("all");
    const [messages, setMessages] = useState<MyMLMessage[]>([]);
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                const msgs = await getMessagesPaginated(
                    page,
                    PAGE_SIZE,
                    filter,
                );
    ...
    return (
        <ModalRoot {...modalProps} size={ModalSize.LARGE}>
            ...
            <ModalFooter>
                ...
                >
                    Total: {totalCount} messages | Showing {messages.length} on
                    this page
                </Text>
            </ModalFooter>
        </ModalRoot>
...
function MessageCard({
    ...
    const authorName =
        author?.username || message.author?.username || "Unknown User";
                ...
                message.editHistory.length > 0 && (
                    <div style={{ marginTop: "8px" }}>
                        <Text
                            ...
                            style={{ color: "var(--text-muted)" }}
                        >
                            📝 {message.editHistory.length} edit(s) - Right
                            click for options
                        </Text>
                    </div>
                )}
